...
-   Introducing [vectors](/content/DEVELOPMENT/2020/courses/DEV-belinsky-2020-ProgrammingFundamentalsUsingR/R/Lessons/1-03_Mathematical_Operations.html#blah)
...
The [script for this lesson is here](https://raw.githubusercontent.com/QFCatMSU/FundProgUsingR/master/scripts/1-04_VectorsAndDataFrames.R)

-   [right-click on the link]{.hl} and choose ***Save link as...***

-   Save to the ***scripts*** folder in your Class Project

 

The data used in this lesson, [twoWeekWeatherData.csv](/content/DEVELOPMENT/2020/courses/DEV-belinsky-2020-ProgrammingFundamentalsUsingR/R/Lessons/../Scripts/twoWeekWeatherData.csv)

-   [left-click the link]{.hl} and save to the ***data*** folder in your Class Project
...
The script for this lesson will not execute correctly if it is treated as a standalone script (i.e., it is executed outside of the RStudio Project). [This is because the file looks for a data file and it needs to know where to look for it.]{.hl}
...
One of the main reasons to create a vector is that [the values in the vector are all associated]{.hl} and we can perform mathematical operations on the whole vector.  For instance, we might have temperatures in Fahrenheit and want to convert all the temperature to Celsius:
...
[RStudio is a text editor]{.hl} so we can open  ***twoWeekWeatherData.csv*** in RStudio by *either*:
...
***weatherData*** is a [data frame variable]{.hl} and it appears in the ***Environment*** tab under ***Data*** as [14 obs. of 5 variables]{.hl}. (i.e., 14 rows and 5 columns), which matches what we saw in the CSV file (*Fig.##*). 
...
3.  ***chr***: (e.g, ***noonCoondition***) this stands for [character]{.hl}, and character is really short for [string of characters]{.hl}, and this in turn is typically shortened to *string*.  Whatever you call it, it is the catch-all category for values R cannot recognize as anything else. 

 

[Note: the **date** column has date values but it is not formatted properly so R does not recognize it as a Date -- something we will cover later.]{.note}
...
Inside the function ***read.csv()***, I included four arguments (***file, sep, header,*** and ***stringsAsFactors***).  [Arguments are like the knobs and dials for a function]{.hl}.  The function reads in a CSV file and the arguments tweak how the file is read.
...
If you execute the script file outside your RStudio Project, you will get a [cannot open file]{.hl} error because the script does not know where to start looking for the CSV file.
...
### Pulling out (subsetting) individual values
...
***highTemp7a***, ***highTemp7b***, and ***highTemp7c*** all show that the high temperature on the 7^th^ day is **60**.
...
***consecVals*** is a vector with 9 values -- the high temperatures from days **3** through **11**.  [Note: **9** values, not (11-3=) **8** -- we include both the 3 and the 11.]{.note}
...
1.  Save the file [DowJones1980_2012.csv](/content/DEVELOPMENT/2020/courses/DEV-belinsky-2020-ProgrammingFundamentalsUsingR/R/Lessons/../Scripts/DowJones1980_2012.csv) to the ***data*** folder in your RStudio Project
...
[Round]{.hl}brackets are used when you are [calling a function]{.hl}*.*
