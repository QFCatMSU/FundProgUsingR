...
In @fig-weatherdataframe there is a weather data frame ([note: this data frame will be used in future lessons]{.note}) where each column has a measured daily weather value (e.g., ***minTemp***, ***maxTemp, relHum***).  Each row gives the measured weather values for a specific day (e.g., row 1 is Jan 1, row 2 is Jan 2).
...
***file = "data/twoWeekWeatherData.csv"*** means that the script opens the file called **twoWeekWeatherData*.csv*** located in the ***data*** folder inside your RStudio Project Folder.
