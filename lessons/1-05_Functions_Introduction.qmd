...
The ***Usage*** part of the ***Help*** page shows that ***median()*** has two arguments:
...
If there is an **NA** value in a vector then, for most mathematical functions in R, the return value will be **NA**. The median of a vector with **NA** values is always **NA_real\_**. **NA_real\_** means that median cannot be determined but would be a real number if it could be determined.
...
***seq()*** has 5 arguments: ***from, to, by, length.out,*** and ***along.with***.  We are going to ignore ***along.with***, which is not needed nor very useful.
...
[note: every argument in ***seq()*** has a default value, meaning you can call ***seq()*** without any arguments. The result is not useful, though – but worth a try.]{.note}
...
For instance, the default value for ***sep*** (the separator between data) is a space in ***read.table()*** and a comma in ***read.csv()***.  ***read.csv()*** uses all the argument from ***read.table()*** and changes some of argument value (the arguments underlined below).
