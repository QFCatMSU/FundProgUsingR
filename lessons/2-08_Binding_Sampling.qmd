...
-   Move 4.5 to extension and lesson in the Lists lesson- K
...
The [script for the lesson is here](../scripts/2-08_BindingAndSampling.R)
...
### Sampling from a data frame \<make an extension instead?\>
...
The mean value, ***mean(randomTempsSeeded)***, isvery close to **30**.
...
You will get this warning when plotting a histogram:[\`stat_bin()\` using \`bins = 30\`. Pick better value with \`binwidth\`]{.hl}.  This warning is kind of GGPlot bug -- GGPlot does not like it's own default of using 30 bins in a histogram.  You can change it if you want, but there is no need.
...
-   If you move the code ***set.seed(12345)*** from line **44** (right before ***randomTempSeeded*** was set) to line **39** (right before ***randomTemps*** is set):
...
-   You probably want to use the ***seq()*** function \<lesson 1.4 section 7.4\>
...
-   the ***mean*** and ***sd*** used to in ***rnorm()*** to create these observationswere both whole numbers.
