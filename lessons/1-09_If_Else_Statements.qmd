...
In the last lesson we looked at execution of the ***if()*** statement and how it changes the flow of a script.  if() statements contain a conditional statement inside the parenthesis and an attached codeblock surronded by curly brackets ( ***{ }*** ).  The codeblock is only executed if the conditional statement is ***TRUE***. 
...
I find the method in @fig-commonStructure harder to read and more challenging to add comments to than the method @fig-structure. However, for this class, you can use either method when coding if-else structures.
...
With an ***if-else-if*** structure you can check as many mutually exclusive situations as you want.  And ***if-else-if*** structures are efficient because they will only execute the code until the first ***if-else-if*** conditions is **TRUE**.
...
A\) If you highlight lines 12-21 of the lesson script and press ***Run***, you get the error [Error: unexpected 'else' in "  else". ]{.hl} In comment explain why you get this error.  The error is similar to what happens in [Curly bracket placement in R and a bug].
...
![Unexpected else error caused by not using curly brackets in if-else structures.](images/R-1-09_unexpectedElse.png)
...
What happens is that the codeblock below was detached from the ***else*** statement. Because of this, ***cat("E")*** will execute every time the script is executed..
