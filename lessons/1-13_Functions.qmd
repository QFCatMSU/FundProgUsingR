...
-   add to this lesson that rm(list=ls()) is not used (and explained more in next lesson)
...
-   Function Script: [download the function script here](../scripts/myFunctions.R)
...
2\) Packages, which are extensions of R (e.g., ***ggplot2***, ***tidyverse*** – we will cover these in the next part of the class)

3\) User-created, which we will focus on in this lesson

## Parts of a function

All functions, whether built into R, in packages, or user-created, have the same four components: **name**, **arguments**, ***codeblock***, and **return value**. 
...
When a caller calls a function, they usually expect to get some information back from the function. In the case of ***mean()***, the caller is expecting the mean value of the vector provided as an argument. The ***Value*** section of @fig-meanFunction gives information about what the function is returning to the caller. ***mean()*** returns a "length-one object".
...
So far we have used functions that are built in to R. Now we will use functions from the function script for the lesson named ***1-13_myFunctions.R*** (in @sec-downloads).
...
[note: ***source()*** and the ***Source*** button both execute all code within a script]{.note}
...
1.  The arguments in the function call are variables used in the codeblock. So, the caller gives a value to ***vec*** and the codeblock uses this value.

2.  Functions explicitly return a value to the caller using ***return()***. In this case, ***meanVal*** is returned.
...
-   Why do we count the number of ***NA***s?

-   For the two ***else if*** statements, what assumption can be made about the i^th^ value being checked? Why can we make this assumption?
...
6\) Create a function that takes numbers from 0 to 100 and returns a grade from A to F.
...
-   Return to caller the number who passed (score \>60) divided by the number of valid values
