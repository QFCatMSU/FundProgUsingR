...
-   put two lines at top of every codeblock? Yes, with explicit understanding that line number only works for function/object not existing when not inside a for loop or function

    -   is a session variable.... save this info for a much later lesson (working directories??)
...
A\) Add this code as the third line in your script (right after the ***rm()*** lines):
...
Challenge question: What would happen if you set the seed before calling rm()?
