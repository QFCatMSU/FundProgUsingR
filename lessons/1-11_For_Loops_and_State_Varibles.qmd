...
But then we would be checking for **Rain** on days that we already know is **Sunny**.  ***noonCond*** on a specific day cannot be both **Sunny** and **Rain** (i.e., they are mutually exclusive) -- so the extra checks are not needed.  The code would still work, just not be as efficient.  Efficiency becomes more important as the number of values in your vector increases -- and efficiency means quicker execution time.
...
2\) Find out ***if any*** of the odd days low teperatures were less than 40 degrees. Hint: use ***seq()***

-   add code to exit the for loop as soon as the condition is met

-   add code that outputs to ***Console*** whether any days meet the condition and, if so, which day

 

3\) Find the mean low temperature using ***for*** loops: get the total and divide by the number of values.
