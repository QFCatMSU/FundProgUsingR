...
-   Change question 1 to be its own question (i.e., subset ... in 4 different ways)
...
The [script for the lesson is here](../scripts/2-10_StatisticalTests.R)
...
We will do a second t-test between years that do look as similar in @fig-box_temp_data -- 2013 (column **3**) and 2016 (column **6**):
...
For ***tTest1*** an ***tTest2***, we used 2 different methods for subsetting a column in a dataframe.  [There are actually seven ways to subset a column in a dataframe]{.hl} and they all produce the same result.
...
-   The ***\[ , \]*** operator is the most explicit because it shows the two-dimensionality of the dataframe -- and allows you to subset the rows along with the columns -- this is the best choice if you are subsetting rows and columns at the same time

-   The ***\[\[  \]\]*** operator is best used for more complex object like ***Lists*** -- I generally avoid these when using data frames

-   The ***\[  \]*** operator does a different kind of subsetting than the other three operators -- this will be explored in the next lesson but this author would argue that it is the least useful operator
...
ANOVAs are functionally similar to a t-test but R requires that the data be structured differently.  In R, the ANOVA function ***aov()*** cannot compare values from different columns like ***t.test()***.  Instead ***aov()*** compares values within a column [grouped by a second column]{.hl}.

 

In other words, to perform an ANOVA using ***aov()***, [you need to use a stacked dataframe]{.hl} and we will use the stacked dataframes created in the last lesson and loaded in from the ***rdata*** file.
...
-   ***data***: the (stacked) dataframe that contains the data

    -   in this case: the stacked dataframe ***stackedDF2***

-   ***formula***: the columns used for the ANOVA in the form: ***all_values\~grouping***

    -   ***all_values*** comes from the ***values*** column and the ***grouping*** comes from the ***ind*** column ([note: this is similar to an **x, y** mapping GGPlot except that it is y\~x]{.note})
...
The results seem to match the linear regression in the plot in @fig-scatterplot .
...
1\) In the application, use at least four different methods of subsetting a data frame from @sec-seven_subsets

 

2\) Based on the boxplot of all 6 years ( @fig-box_temp_data ),  try to figure which three years would most likely come form the same distribution (i.e., the ANOVA would not reject the NULL hypothesis).
...
5\) For only the linear model that shows the most correlation of the four above:

-   print to the ***Console*** and, in comments, explain the summary of the linear model
