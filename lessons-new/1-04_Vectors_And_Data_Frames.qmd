...
- Saving columns from a data frame
...
- Save the linked script file to the ***scripts*** folder in your Project Folder
...
In the last two lessons, everything we did was contained to one script file -- no outside file was needed.  The scripts in those lessons were standalone scripts and could be executed inside or outside your Project Folder with the same results.

 

The vast majority of script are not standalone, they rely on other files – especially data.

 

The script for this lesson will not execute correctly if it is treated as a standalone script (e.g., it is executed outside of the Project Folder). [This is because the file looks for a data file and it needs to know where to look for the data file.]{.hl}

### Set Working Directory (a cautionary statement)

Many R programmers do not use Project Folders. This causes all sorts of problems when a script file look for other files.  The old way to get around this is to set the Working Directories using ***setwd()***.  This is a mess because Working Directories change when files are moved, whether between computers , within a computer, or on GitHub.  [Scripts that set working directories are much harder to share and maintain.]{.hl}  This author strongly suggests that you use Project Folders to avoid working directory issues.
...
After executing the code in this section, we can see in the ***Variables*** section of the ***Session*** tab that ***tempHighCel*** has 5 converted temperatures and that ***tempMean*** is the same as ***tempAvg***:
...
![View of temperature values in Variables](images/R-1-04-TempValue-in-Env-2.png){#fig-environment-view .fs}

[note: ***dbl \[5\]*** means there are 5 double values in the vector, ***int*** stands for integer]{.note}
...
Putting values into vectors also allows us to associate values between vectors.  If we have a vector of high temperature values, then we could also have a corresponding vector of low temperature values.
...
The ***Variables*** section of ***Session*** shows ([note: after this, I will simply call this ***Variables***]{.note}):
...
After Sourcing the lesson script, there is one data frame named ***weatherData***. In ***Variables***-\> ***Data***, ***weatherData*** is shown to have 14 rows x 5 columns and if you click on the down arrow, you can see the 5 columns. Double-clicking on ***weatherData*** opens up a viewer tab where the data frame is visually represented ( @fig-weatherdataframe ). Each row gives the measured weather values for a specific day (March 27 to April 9). [Note: the extra statistics that are crossed out in the viewer will not be used in this class and not shown in subsequent images.]{.note}

![A data frame viewed in Positron with 5 variables (i.e., the 5 columns, which are vector variables)](images/R-1-04-WeatherDataFrame.png){#fig-weatherdataframe .fs}

## CSV files and data frames

The data in ***weatherData*** comes from the CSV file, ***twoWeekWeatherData.csv***. CSV stands for Comma Separated Values, meaning that the values in the file are (typically) separated by commas.
...
[Note: CSV files are often opened by default in Excel and this is problematic]{.note}: [Trap: Opening CSV files in Excel]
...
[Positron is also a text editor]{.hl} so we can open and edit ***twoWeekWeatherData.csv*** in Positron by clicking ***Open -\> Open File...*** and navigating to the file. When first opened, Positron senses it is a data frame and opens it up in a visual representation. You can view the pure text file by clicking on ***Open as Plain Text File*** (@fig-openCSV-Positron)

![The twoWeekWeatherData.csv file opened in Positron](images/R-1-04_OpenCSV-Positron-2.png){#fig-openCSV-Positron .fs}
...
To use the data in a script we need to first read the data into a script file -- and for that we will use the ***read.csv()*** function.
...
***weatherData*** is a [data frame variable]{.hl} and it appears in ***Variables*** under ***Data*** (@fig-weatherdataframe), which shows the same information that was in the CSV file ( @fig-openCSV-Positron)

### Column value type

In the ***Variables***, when you click on the arrow next to the left of ***weatherData*** (@fig-weatherdataframe) you see some information about the columns in the data frame. The first piece of information is the name of the column (i.e., the header -- ***date***, ***highTemp***, etc.).  The second is the type of value the column contains.
...
[Note: the **date** column has date values but it is not formatted properly so R does not recognize it as a Date -- something we will cover much later in the class.]{.note}
...
***read.csv()*** is called with a few modifiers, called [arguments]{.hl}: ***file, sep, and header***. ***read.csv()*** reads a CSV file and the arguments tweak how the CSV file is read. We will talk a little bit about these arguments here and save the rest of the discussion for the next lesson.

### file

`file = "data/twoWeekWeatherData.csv"` means that the script opens the file called **twoWeekWeatherData*.csv*** located in the ***data*** folder inside your Project Folder Folder.

 

[This only works if you are in the]{.hl} Project Folder because the Working Directory is set to your Project Folder.

 

If you execute the script file outside your Project Folder (@fig-open-outside-project), you will get a [cannot open file]{.hl} error in the ***Console*** because the script does not know to find the CSV file.

![You get a cannot open file error if you try to execute the script outside of your Project Folder](images/R-1-04_OpenOusidePF.png){#fig-open-outside-project .fs}
...
***header = FALSE*** means that the first row contains data (i.e., there are no column names).  If you set ***header=FALSE*** in this example then R will think that the column names are the first value in the vector.  This means you will have 15 values in each vector and the first value will not make sense.
...
In ***Variables*** square brackets ( **\[ \]** ) indicates the variable is a **vector**.  ***highTemps*** appears in ***Variables*** as a ***vector***.  ***highTemps*** is an integer (***int***) that has 14 ( ***\[14\]*** ) values, representing the temperature for each of the 14 days. 

![The highTemps vector variable in Variables](images/R-1-04-VectorFromDF.png){#fig-high-temps .fs}
...
I will be referring quite often to values in ***Variables*** after the script has been sourced (as in @fig-high-temps) .  Instead of taking a screenshot of ***Variables*** (which is hard to maintain), I will display the variables I am focused on like this:
...
***Variables*** shows that ***highTemp7a***, ***highTemp7b***, and ***highTemp7c*** are all equal to **60**
...
If we want to get the high temperature for consecutive days, we can use the sequence-by-one ( **:** ) operator -- often called the [simple sequencing operator]{.hl}. The sequence-by-one operator ( **:** ) gives you every integer number between the two values.
...
::: {#fig-subsetDown}
``` {.r tab="Environment"}
consecVals:     54 40 39 ...    int [9]
consecValsRev:  39 44 55 ...    int [9]
```

int \[9\] means 9 values are in each vector -- the first few values are shown
:::

### Using the Console to view values in a vector

***Variables*** is good for quickly looking at values, but it only shows a few values from each vector.  If you want to see all the values then you can use the ***Console*** tab (bottom-left corner).  In the ***Console*** tab, type in the variable you want to see and press ***Enter***:

![Using the Console tab to view variables](images/R-1-04-ViewInConsole.png){#fig-console-variables .fs}
...
We can use ***seq()*** to create a more complex subset for a vector.
...
::: {#fig-sequences}
``` {.r tab="Environment"}
highTempSeq1:  57 54 39 60 ...    int [7]
highTempSeq2:  75 54 44 53 ...    int [7]
highTempSeq3:  57 40 60 44        int [4]
```
...
1.  Save each of the two columns ***precipitation*** and ***lowTemp*** from **weatherData** to a variable
...
1.  Save the file [DowJones1980_2012.csv](../data/DowJones1980_2012.csv) to the ***data*** folder in your Project Folder
...
3.  Plot the data using the line: ***plot(dowJonesData\$V2)***
    ...
    - If you opened the file correctly the plot will be in the ***Plots*** pane and look like this:

      ![Plot of the Dow Jones data](images/R-1-04_Plot.png){#fig-dowJonesPlot .fs}
...
![Opening a CSV file in Excel -- the possible data loss warning is something you want to heed](images/R-1-04_Excel.png){#fig-excel .fs}
