...
When you execute a script in Positron, all of the current objects are put in the ***Variables*** tab. This is poorly names because, technically speaking, only vectors are variables. The tab would be better named ***Objects***. The ***Variables*** tab is divided into three sections: DATA, VALUES, and FUNCTIONS. These three categories are not a mapping to the three types of objects in R:

- **FUNCTIONS**: holds all function objects (this is a direct mapping)

- **DATA**: holds all data frames and matrices

  - Essentially, any data that can be represented in two dimensions.
