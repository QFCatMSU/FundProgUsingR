...
- Talk about procedural, functional, and object-oriented programming??
...
- [(atomic) vectors]{.hl}: objects that hold values of the same type. The word ***atomic*** is rarely used but does occasionally appear as in @fig-errorAtomicVec.
...
When you execute a script in Positron, all of the current objects are put in the ***Variables*** tab. This is poorly names because, technically speaking, only vectors are variables. The tab would be better named ***Objects***. The Variables tab is divided into three sections: DATA, VALUES, ans FUNCTIONS that somewhat map to the three object types:

- FUNCTIONS: holds all function objects

- VALUES: holds all vectors except 2D vectors ([note: yes, this is a bit confusing!]{.note})

- DATA: holds all matrices (2D vectors) and Lists (which includes data frames)

![Objects in the Positron Variables tab (lesson 1-15 script)](images/R_2-12_ObjectInSession.jpg){#fig-object_in_var .fs}

### Relationship between the objects

In R, vectors, functions, and lists are like different types of files and folders on a computer. A vector is like a data file, a function is like an executable file, and a list (covered in the next lesson) is like a folder: a container that stores objects, including vectors, functions, and even other lists.
...
A vector is a named object that hold values of the same type (or class) -- usually string or numeric.
...
In ***Variables***, ***temperature*** is listed as a double (***dbl***) without any number but this is equivalent to ***dbl \[1\].***
...
And, ***Variables*** shows that it is a double (dbl) vector with eight values \[8\]:

``` {.r tab="Variables"}
JulyTemps:  90 88 86 77 81 83 80 80    dbl [8]
```
...
In ***Variables***, ***rainyDaysInJuly*** is given as a ***lgl***, which stands for ***logical*** -- which is just another name for Boolean:

``` {.r tab="Variables"}
rainyDaysInJuly:  FALSE FALSE TRUE ...   lgl [8]
```
...
The ***Console*** reflects the dimensions in ***JulyTemps2D***: whereas, in ***Variables***, ***JulyTemps2D*** appears as a matrix:
...
In the ***Console***, the third dimension is show vertically as multiple 2D arrays:
...
***class()*** is a function that allows you to either view or set the class of an object (i.e., change a numeric vector to a character vector).  [However, you should never use **class()** to changes the class of an object, instead use a specialized function like **as.character()**.]{.hl}
