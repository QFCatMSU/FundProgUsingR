...
- Opening, and using data from, rdata and rds files

- Performing t-tests and ANOVAS

- demonstrate subsetting methods

- calculate and plot linear regressions
...
In most of the previous lessons, we use ***read.csv()*** to open a CSV file and save the data to a data frame.  In this lesson, we are going to use some of the data frames created in the last lesson -- data frames that were saved to a ***rdata*** file in the ***data*** folder of our project.
...
We use ***load()*** to open the ***rdata*** file and save it to ***Variables***:

``` r
load(file = "data/tempDFs.rdata");
```

Inside the ***rdata*** file is a ***List*** object, ***temperatureDFs***,  with four data frames.  In the next couple lessons we will be dealing a lot more with navigating and creating ***List*** objects.  For now, a ***List*** object is a container for other object (like data frames). ***load()*** takes the objects from the ***rdata*** file and puts in ***Variables***:
...
This put the data frames directly in ***Variables***:

``` {.r tab="Variables"}
⮞ lansJanTempsDF:   [31 rows x 6 columns] <data.frame>
⮞ stackedDF:        [186 rows x 2 columns] <data.frame>
⮞ stackedDF2:       [62 rows x 2 columns] <data.frame>
⮞ stackedDF3:       [124 rows x 2 columns] <data.frame>
```
...
In ***Variables***, ***tTest1*** is labelled as an [htest]{.hl}, which uses a standardized List object to structure the output of a [hypothesis test]{.hl}.

``` {.r tab="Variables"}
> tTest1:  [statistic: 4.77...]   «htest»
```

There is a lot of information in that List and we will dig more into this later.  For now, we can get a helpful summary using the function ***print()*** and passing it the results of the t-test:
...
And, summarize the results of the t-test in the ***Console*** tab using `print(tTest2)`:
...
- The ***\$*** operator is the most convenient because it requires the least code and both Positron and RStudio will give you suggestions
...
- The ***\[\[  \]\]*** operator is best used for more complex object like ***Lists*** (in a later lesson) -- I generally avoid these when using data frames
...
ANOVAs are similar to t-tests except they are generally used to test the means from three or more groups of data, whereas a t-test can only test the means from two groups.  For comparison, we are going to start by using an ANOVA on 2 groups (Jan2013 and Jan2016).
...
The ***residuals()*** function gets the 124 residuals, which is the difference between the 124 observations and the mean, from ***Jan4MonthAnova:***
...
``` {.r tab="Variables"}
residuals: 28.387 -0.613 7.387 6.387 ...    dbl [124]
```

We can then plot the ***residuals*** vector in a histogram by mapping it to ***x*** ([note: in a histogram, the y-axis is the count of unique values mapped in **x** as does not get its own mapping]{.note}):
...
The argument we need to set for ***lm()*** is ***formula*** and ***formula*** is in the form: ***y \~ x***
...
The two types of files serve a similar purpose: saving objects from ***Variables*** to your Project Folder to be used later. The big difference between the two is how they are later loaded into ***Variables***.

 

When you load an rdata file, the objects in the rdata file get loaded directly into ***Variables*** exactly as they were (i.e., same name and values). In this case, the List ***temperatureDFs*** is directly loaded into ***Variables***:

``` r
load(file = "data/tempDFs.rdata");
```

When you load an rds file, the object in the rds file need to be saved to a variable to get loaded into ***Variables***. This means the values are the same but the name could be different. In this case, the information from ***temperatureDFs*** is save to a variable named ***tempDFs2***:
...
RData files are the quick-and-dirty way to save R objects and reload them later. A single `.RData` file can store multiple objects, or even all object in ***Variables***. This is convenient for short-term work, for example restoring your ***Variables*** tab the next morning. However, loading an `.RData` file restores objects directly into ***Variables*** can have unintended consequences: existing objects with the same names will be overwritten. This makes `.RData` files risky in larger scripts and collaborative settings.

 

RDS files are the safer and more collaborator-friendly way to save objects. An `.rds` file stores exactly one object. The file contains the object’s data, but not its name. When loading the file, the caller explicitly assigns the object to a variable name of their choosing. This explicit assignment avoids accidental overwrites, and leads to workflows that are easier to reproduce and maintain.