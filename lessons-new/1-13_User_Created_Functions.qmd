...
So far we have used base-R functions. Now we are going to create our own (user-generated) functions. In general user-created functions are put in a separate script so that the functions can be used by multiple scripts – just like base-R functions can be used by any script.

 

Because the functions are in a separate script, we must first read (i.e., ***source***) the functions script from the main script before we can use the functions. This line in ***1-13_Functions_Main.R*** executes all code within the script ***1-13_Functions_Only.R*** (assuming it is in the ***scripts*** folder inside your project folder):

``` {.r tag="1-13_Functions_Main.R"}
source("scripts/1-13_Functions_Only.R");
```

R will put the three functions from ***1-13_Functions_Only.R*** into ***Variables*** -\> ***Functions***:
...
![Function list inside Variables](images/R-1-13_FunctionList.png){#fig-functionList .fs}

Functions, like variables, are named objects that store information. The difference is that [variables store values]{.hl} whereas [functions store script]{.hl}. User-generated functions must be in ***Variables*** before they can be called.
...
The purpose of this code is to clear out the environment (***Variables)*** so that your script has a clean slate. In reality, it is to force programmers to include all the resources they need to run a script inside the script they are running (i.e., make the script self-contained). However, function scripts are resources for the main script and calling `rm(list=ls())` from the function script would clear the variables declared in the main script. [A function script should never clear the environment]{.hl}.
...
[Extension: functions with implicit return values]

### Setting argument values

***hello_you()*** is in ***Variables*** so we can now call it in ***1-13_Functions_Main.R***. Let's first call the function using only the ***who*** argument:
...
[Note: if ***hello_you()*** is in ***Variables***, then you can also call it in the ***Console***:]{.note}
...
Looking at the ***Help*** for ***boxplot()*** (`?boxplot` in ***Console***), we see that the **Default S3 Method** has many arguments but all of them, except ***x***, has a default value. ***x*** is the data, which the caller needs to provide, and the rest of the argument are optional tweaks to the function.
...
In general, I would argue that arguments beyond the first should be named for readability.
...
In ***Variables***, we see that ***mean_simple()*** has one argument, ***vec***, that does not have a default value.
...
Each call to the function ***mean()*** passes in a vector of values for the ***vec*** argument and saves the return value to a variable (***ret2a***, ***ret2b***...). In ***Variables*** we can see the return values stored in the variables.
...
Each line of code above calls the function ***mean_advanced()***, passes in values for one or more arguments, and saves the return value from the function to a variable (***ret3a***, ***ret3b***...). In ***Variables*** we can see the return values stored in the variables.
...
c.  Make sure you thoroughly test all the functions you create in your main script. [I want to see the test code in ***app1-13.r***]{.hl}.
...
3\) Create a function that take a vector of numeric values and returns either the standard deviation or variance of the values:

- There should be two arguments: (1) the numeric vector and (2) something that allows the caller choose whether they want the standard deviation or the variance of the vector.

- Make standard deviation the default for the second argument
...
![Name, arguments, and return value for the function mean() in the Help tab](images/R-1-13_FunctionComponents.png){#fig-meanFunction .fs}
...
In ***Variables*** we can see the return values stored in the variables.
...
You quite often see R functions without any ***return()*** statement. This is because R functions implicitly return the last called variable. So ***mean_simple()*** could be written like this:
...
{ 
  ...
  ### Divide the total value by the number of values to get the mean
  meanVal = vecAdded / length(vec);  # meanVal is implicitly returned ... because it is the last called variable
} 
```

Or, often, you will see this:
...
{ 
  ...
  ### implicitly return the mean value to the caller
  meanVal;
} 
```

I consider this a bad habit – when programming, you want to be explicit with your code. That means if you want to return a value to caller, you explicitly say that you want to return the value by using ***return()***. This both avoids confusion and allows for more robust functions – we will see in future lesson that there can be multiple ***return()*** in a function.