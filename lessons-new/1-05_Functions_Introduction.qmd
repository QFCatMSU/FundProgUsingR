...
- Using help in Positron to find information about functions
...
- Save the linked data file to the ***data*** folder in your Project Folder
...
[note: we usually put ***( )*** after the name of a function to differentiate it from a variable]{.note}

 

Functions are standalone scripts designed to do some repeatable operations.  A function almost always takes [arguments]{.hl}, which are variables used by the function that are set by the caller, and sends back to the caller a [return value]{.hl}.
...
The arguments are the values for ***file, sep,*** and ***header***.

The return value is the data from the CSV file and gets saved to the variable ***weatherData***.
...
The arguments are the values for ***from, to, and by***.

The return value is the sequence of numbers that gets saved to the variable ***seq1***.

## Arguments

In programming, the values you pass into a function (e.g., ***file, sep,*** and ***header***) are called [arguments]{.hl} of the function.  Arguments are added to the function call to tweak the operation of the function.

 

[Note: The majority of the programming world uses the term **parameters** instead of **arguments**.]{.note}  I tend to like ***parameters*** better as I think it better indicates how these are properties of the function.  But, R is a programming language for statisticians and ***arguments*** is a mathematical term, so R chose to go with ***arguments***. Just note you will occasionally see the term parameters, and it means the same thing as arguments.
...
And ***Variables*** shows the return value, saved to ***medianVal1***, is **6**
...
In Positron, you can access the Help pages for functions by typing `?function_name` in the Console. So, for the case of median:

``` {.r tab="Console"}
?median
```

When you hit ***enter***, the ***Help*** tab in Positron opens up to the help page for ***median()***:

![The median function in the Help tab in Positron](images/R-1-04_MedianHelp.png){#fig-median .fs}
...
- ***na.rm***: a TRUE/FALSE value that determines how to handle ***NA*** values in the vector. **NA** means Not Available and usually indicates a problem with the data.

 

[Note: the ( ... ) in ***Arguments*** can be ignored -- it is R indicating that this function can be expanded with more arguments]{.note}

#### Help tab for function

The ***Help*** tab is a useful resource for finding out more information on functions. The information that appears in Help is from the latest R documentation. If you do an internet search, you will often get older documentation. [Note: the Help window is using the website <https://search.r-project.org/>.]{.note}
...
The other argument in ***median()***, ***na.rm***, has a default value of ***FALSE*** . This means that [you do not have to supply a value for the argument]{.hl} ***na.rm*** when you call the function.  You only need to supply a value if you want to change ***na.rm*** to something other than the default, **FALSE** (e.g., **TRUE**).

 

Our first example did not have any **NA** values, let's create a vector with an **NA** value in it:
...
If there is an **NA** value in a vector then, for most mathematical functions in R, the return value will be **NA**. The median of a vector with **NA** values is always **NA_real\_**. **NA_real\_** means that median cannot be calculated but would be a real number if it could.
...
My general recommendation is to always use argument names beyond the first agument (which is usually reserve for the data) when calling a function.  This makes your code easier to read and you don't have to worry about the order of the arguments.  This is especially important when you are dealing with more complicated functions that have lots of arguments (e.g., plotting functions).

## Help seq()

Let's look at the function ***seq()*** by typing `?seq` in the ***Console*** and pressing ***enter***.

``` {.r tab="Console"}
?seq
```

![The Help page for the seq() function](images/R-1-04_SeqHelp.png){#fig-seq .fs}

 

The ***Help*** for functions can contain many abstractions and extraneous information (especially for beginners). Throughout this course, we will open up the ***Help*** menu for new functions to get you more familiar with the abstractions.
...
***seq()*** has 5 arguments: ***from, to, by, length.out,*** and ***along.with***.  We are going to ignore ***along.with***, which is not needed.
...
[note: every argument in ***seq()*** has a default value, meaning you can call ***seq()*** without any arguments. The result is not useful, though – but it is worth trying.]{.note}
...
``` {.r tab="Variables"}
seq2:   1 3.25 5.5 7.75 10    num [5]
```

## read.csv() expanded

When you go to the ***Help*** page for ***read.csv()*** (@fig-readcsv), you are taken to the ***Help*** page for ***read.table()***. This is because ***read.csv()*** is a modified version of the function ***read.table()***. In other words, ***read.csv()*** starts with all of the arguments/values in ***read.table()*** and then modifies some of them.

 

For instance, the default value for ***sep*** (the separator between data) is a space in ***read.table()*** and a comma in ***read.csv()***.  ***read.csv()***. ***header*** is used by default (***FALSE***) in ***read.table()*** but are (***TRUE***) in ***read.csv()***.
...
![The Help page for read.table() contains the help for read.csv()](images/R-1-04_ReadCsvHelp-2.png){#fig-readcsv .fs}

[note: I am not sure why ***read.csv()*** repeats arguments that have the same values in ***read.table()*** (e.g., ***quote***, ***dec***)]{.note}

### read.csv arguments

When you call ***read.csv()***, you can use any of the argument from ***read.table()***. For instance ***allowEscapes*** and ***stringsAsFactors*** are arguments listed only in ***read.table()***, but they can be used in ***read.csv().***

 

There are a lot of arguments in ***read.table().*** The majority of these arguments rarely need to be changed from their default value.
...
***file*** is an argument that is not set equal to anything in ***read.table()*** or ***read.csv()***.  This is because ***file*** must be supplied by the user --[**file** is (almost always) the only required argument]{.hl} in either of these functions*.*  This should make sense as there is no point in calling ***read.table()*** / ***read.csv()*** without a data file.
...
If you look in ***Variables*** the values for ***weatherData*** and ***weatherData2*** are exactly the same.

![Expanding weatherData and weatherData2 in ***Variables***](images/R-1-05-weatherdatas.png){#fig-weatherdata .fs}

### Arguments without default values

There are four arguments in ***read.table()*** without default values (@fig-readcsv) : ***files***, ***row.names***, ***col.names***, and ***text***. You need to read the description in the argument section to find out how the function determines these values. For ***text***, the description reads:
...
 \
This mean that you can pass in a character string as the data instead of using a file. This is very rare, hence why ***text*** is near the bottom of the arguments. So, technically speaking, [either ***file*** or ***text*** is required to have a value]{.hl}.
...
2\. Explain, in comments, why you get an error if you try to use all four arguments in ***seq()***: ***from, by, to, length.out***

 

3\. In comments answer: In ***read.csv()***, what are the default values for ***row.names***, ***na.strings***, ***fill*** , and ***comment.char***?

[Note: one argument is defined in ***read.table()***, two are defined in ***read.csv(),*** and one you need to read the description.]{.note}
...
6\. Call the function ***factor()*** twice, passing in (1) the column ***date*** and (2) the column ***noonCondition*** as the argument and answer the following in comments:
