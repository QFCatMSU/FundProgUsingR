...
***file*** is an argument that is not set equal to anything in ***read.table()*** or ***read.csv()***.  This is because ***file*** must be supplied by the user --[**file** is the only required argument]{.hl} in either of these functions*.*  This should make sense as there is no point in calling ***read.table()*** / ***read.csv()*** without a data file.
...
3\. In comments answer: In ***read.csv()***, what are the default values for ***row.names***, ***na.strings***, ***fill*** , and ***comment.char***? [Note: some are defined in ***read.table()***, some are defined in ***read.csv()***.]{.note}
