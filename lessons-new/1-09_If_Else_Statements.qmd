...
In the last lesson we looked at execution of the ***if()*** statement and how it changes the flow of a script.  ***if()*** statements contain a conditional statement inside the parentheses and an attached codeblock surrounded by curly brackets ( ***{ }*** ).  The codeblock is only executed if the conditional statement is ***TRUE***. 
...
Many times when you are programming you want to execute something when the condition (e.g., temperature \> 50) is **TRUE** and something different when the condition is **FALSE**.

 

You can execute a codeblock on a **FALSE** condition by using an ***if-else*** statement:
...
@fig-structure uses the bracket placement I prefer. However, when you see an R script you often see people write their ***if-else*** statements like this:
...
[Note: Neither method follows programming standards that you will find in most other language.]{.note} More information about this is here: [Extension: Programming standards for if-else statements]
...
Conditional operators can also be used to compare two string values.  Usually, we are using ***==*** or ***!=*** to compare string values.  So, the ***else*** statement can be used for the opposite condition:
...
- You are executing code that you know does not need to be executed.  ***Sunny*** and ***Not Sunny*** are mutually exclusive -- there is no need for the script to check both.  This extra check adds a little bit to the script's execution time and this can make a difference if you are checking 100,000 values in a data frame.

- An ***if-else*** statement is easier to debug than two individual ***if()*** statements because there is only one conditional statement to debug (as opposed to two).

## Adding in more conditions: if-else-if Structures

We are limited to two outcomes when using ***if-else*** structures: (a) the ***if()*** condition is ***TRUE*** or (b) The ***if()*** condition is ***FALSE***.  However, the examples above contain situations that could easily have more than two possible conditions.  For instance, ***noonCondition*** can be "Sunny", "Cloudy", or "Rain" -- and we can check for all three in an ***if-else-if*** structure.

 

With an ***if-else-if*** structure you can check as many mutually exclusive situations as you want.
...
***if-else-if*** structures are efficient because they quit once they find a conditional statement that is ***TRUE***.

 

In @fig-multConditions, if we replaced day 4 with day 2, which was "Cloudy", then only the first condition (***noonCond\[4\] == "Cloudy"***) gets checked.  Because "Cloudy" is **TRUE** there is no need to check "Sunny" and "Rain".  If there are thousands of values to check (e.g., you are checking all the values in a large vector -- next lesson) then a lot of computational time is saved by not checking conditions that cannot be **TRUE**.

## The error, or everything else, condition

We often want to execute something for values we have not prepared for.  For instance, a phone system might have menu options numbered **1** through **5**.  If the user presses **7**, the system will usually have a message saying something like "sorry, **7** is not an option".

 

The problem with the script above is that it does nothing if the condition for the day is not given anywhere in the ***if-else-if*** structure (i.e., all the conditional statements are **FALSE** *-* perhaps the condition was ***Fog***).

 

In an ***if-else-if*** structure you cannot anticipate everything that somebody will input into a dataframe -- let alone all of the errant spellings.  To resolve this, we create what is often referred to as an [error condition]{.hl}*,* meaning we set up a codeblock that gets executed if none of the conditions in the ***if-else-if*** structure are ***TRUE***.  This error codeblock gets attached at the end to an ***else*** statement.
...
The ***else*** statement guarantees a response from the script – no matter what ***noonCond*** is:
...
Adding the error statement also helps you debug your code.  In this case, the error statement tells you that some value does not meet any of your conditions. The codeblock attached to the ***else*** statement can be used to provide useful information about what went wrong -- and perhaps suggest a fix.
...
An **if-else-if** structure ends as soon as a condition is determined to be **TRUE**. This means that [at most one codeblock in an ***if-else-if*** structure gets executed]{.hl}.  In the above example (@fig-numberRanges), if the temperature is 75 (i.e., day 14), the first condition (***highTemps\[14\] \> 70***) is ***TRUE*** and "That is hot for April" is output. The **if-else-if**  ends here and none of the other conditions are checked.

 

The second condition (**highTemp\[14\] \> 60**) would, if checked, also be **TRUE**. But this condition does not get checked because a condition above was already **TRUE** -- the ***if-else-if*** structure stopped there.
...
B\) In comments answer: Why might it be easier to find errors in a dataset containing weight values using ***if-else-if*** structures vs. just ***if()*** statements? Give examples.
...
- hint: [do the if-else-if structure strictly in ascending order or strictly in descending order]{.hl}.  This mean splitting the "invalid" category into two conditions (less than -20 and greater then 100)\
...
You will often see R scripts that use ***T*** and ***F*** as shortcuts for ***TRUE*** and ***FALSE***.  R accepts ***T*** and ***F*** as shortcuts for ***TRUE*** and ***FALS*** [but T and F are not keywords]{.hl}***.***  This means that someone can assign a value to ***T*** or ***F*** that overrides the default ***TRUE/FALSE*** values – as done in @fig-keyword-error. If this happens, using ***T*** or ***F*** as shortcuts for ***TRUE*** or ***FALSE*** would produce unexpected results.  It is best to stick with the reserved (and protected) keywords ***TRUE*** and ***FALSE***.
...
The only advantage to not using curly brackets is that it takes up less space.  I recommend that you include curly brackets even if there is only one command in the codeblock -- it is more explicit and you avoid errors later on.
...
A common mistake for people new to programming is to treat ***else*** like an ***if()*** or an ***else if()*** and attach a condition to it.  The codeblock attached to the ***else*** statement is supposed to be executed [if all other conditional statements in the ***if-else-if*** structure fail]{.hl}.
...
What happens is that the errant condition ***(score \<= 60)*** was attached to the ***else*** instead of the codeblock. Since the codeblock below was was detached from the ***else*** statement, it execute without condition (i.e., it executes everytime).
...
I would argue that these methods above improve upon the R standard for coding ***if-else*** structure. The problem is that, in R, the above methods will occasionally cause an [unexpected 'else']{.hl} error. The reason why gets deep into the weeds of R.