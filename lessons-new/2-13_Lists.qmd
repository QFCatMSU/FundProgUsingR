...
- expand lesson to include reading/writing Lists to RDS file (save lm to RDS – use saveRDS() and laodRDS() – Help
- talk about attributes on a matrix vs data frame
  - how matrix (dim) and data frame determine rows and columns
- simpler data frame?
...
- Vectors (sometimes called atomic vectors): objects that hold multiple values of the same type (e.g., numeric, string, logical)
...
- the ***weatherData*** dataframe from previous lessons – but reduce the data to make it easier to read in ***Variables***
...
Matrices and data frames are both 2D structures that contain data – but they are formed very differently. [A matrix is a single vector]{.hl} object with the ***dim*** attributes modified to define how many rows and column are in the data. [A data frame is structured like a folder]{.hl} (i.e., List object) that holds multiple vectors of the same size – the vectors can be of different types (e.g., string, Boolean, numeric, integer, Datetime).

 

In a matrix, the number of rows and columns are defined by the ***dim*** attribute that is set when you declare the matrix using the ***nrow*** and ***ncol*** arguments. In a data frame, the number of columns is the number of vectors in the List/folder and the number of rows is the number of values in each vector, which must be the same.
...
The ***Console*** also clearly shows that these are not number, but string values consisting of numbers. So, you can refer to the row using the index number or the row name. Here we are extracting row 4, or "153":
...
A List is an object that holds other (usually related) objects, much like a folder holds files. A data frame is a special type of List. Lists are often used to organize and send data. For instance, when you perform a statistical test, the results are often stored as Lists, as we will see later in the lesson.

 

A List is usually used as a data structure that contains vectors and subLists. A List can also include functions – this is often done when programming using the Object-Oriented Programming (OOP) paradigm, a topic not covered in this class, but you will see in some advanced packages (e.g., shiny, terra, RTMB).
...
If we want the objects inside the List to have names, then we have to specify a name while creating the List ([note: you can use a different name]{.note}).  You do that by setting a name equal to the object when calling ***list()***:
...
In the previous example, all the objects were put in the List at once.  What often happens is data is generated and gets dynamically added to a List, so the objects are put in the List separately.  We are going to create a List equivalent to ***list2*** except the three object will be put in one at a time.
...
When you are automating the generation of a List, you often do not want to name objects, you just want to keep appending objects to the List. Here we are saying that the 1^st^ object in ***list4*** is ***someAnimals***, the 2^nd^ object is ***someNumbers***, and the 3^rd^ object is ***weatherData_reduced***:
...
However, appending to a list is often done in a ***for*** loop where you are adding data to a list each cycle. In this case, 10 random numbers are chosen each cycle of the ***for*** loop and then added to ***list6***:
...
3)  by index number – [this is the only option when objects do not have a name]{style="hl"}

... and we can combine these.

 

Here are various ways to subset ***list3*** -\> ***weather*** -\> ***minTemp***
...
[Note: \$ is generally easier to use than \[\[ \]\] because it is less typing and Positron gives suggestions, but \$ can only be used with name, not index values.]{.note}

 

The ***Variables*** tab does a good job of showing the structure of a List object:

![Variables tab in Positron showing the structure of a List object](images/R_2-13_ListInVariables.png)
...
***model1*** is a List object with the information for the linear model:
...
We need to use **\[ \]** to subset ***coefficients*** because ***coefficients*** is a vector. (Intercept) is the name of the value inside the vector. We could also subset by the index number:
...
If you look through ***model1***, you will find two new variable types: ***qr***, ***language***, and ***terms***.  ***qr*** is a special type of List (like data frames and lm). The other two are more advanced types of objects that you do not have to worry about until you get a lot more advanced in R – even Positron cannot read a ***language*** object – hence the ***??***.
