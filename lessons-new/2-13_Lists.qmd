...
- procedural, functional, and object-oriented programming discussion
...
A List is an object that holds other (usually related) objects, much like a folder holds files. Lists are often used to organize and send data. For instance, when you perform a statistical test, the results are often stored as Lists, as we will see later in the lesson.

 

"Most lists simply organize data, but more advanced packages often return lists that contain both data and functions. These functions know how to operate on the data stored in the object."
...
### Data frames are Lists, matrices are vectors

Notice that, in the Environment, ***weatherData*** has an arrow to expand it whereas ***someAnimals*** and ***someNumbers*** contains all of their information in one line.  It is this expand arrow that indicates ***weatherData*** is a List, whereas ***someAnimals*** and ***SomeNumbers*** are atomic vectors.  ***weatherData*** is a data frame, which is a List where all the objects inside are vectors of the same length:
...
\<Show the Positron Variables tab\>

\<Show three ways to subset
...
called lm – in Console: typeof(model1)
...
D\) Create one List named ***app2_13_List*** that contains the objects from parts A, B, and C in this application

 

E\) Write the List to an RData file named ***app2-13List.RData*** in the ***data*** folder.
