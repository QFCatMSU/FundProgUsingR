---
title: "2-06: Regular Expressions"
short: "2-06"
...
- redo Q5 to invert part of the command
...
The data file for this lesson a made up list of 75 file names. Let's save the file names column to a vector:
...
![The file names dataframe](images/R_2-06_FileNameDF.png){#fig-filenames .fs}
...
We are going to step up in difficulty and check for something specifically at the beginning and the end of a string. In RegEx, the caret ( \^ ) is a special character that indicates the beginning of a string and ( \$ ) indicates the end of a string.

 

So, to say "starts with ***station***" as opposed to "contains ***station***", we put ***\^*** at the beginning to indicate that the string needs to begin with the pattern:
...
``` r
start_end_bad = grep(file_names, pattern="^station&txt$")   # does not work!
```

In this case, we need to be explicit about what is in between ***station*** and ***txt*** – which can by any number of any characters. In Regex we use [**.\***]{.hl} to repesent [any number of any characters]{.hl}. The pattern is `^station.*txt$` and says:

- starts with ***station*** ( `^station` )

- followed by any number of any character ( `.*` )

- and ending with ***txt*** ( `txt$` )
...
In Regex, a [dot]{.hl} ( `.` ) [represents any character]{.hl}. We can also search for a character within a set or range of characters.
...
**\[ \]** means that RegEx is looking for one of any of the characters inside, it does not matter the order of the characters inside the brackets. This ***pattern*** changes the order but does the same thing:

``` r
year_2016_2019b = grep(file_names, pattern="201[7986]")
```

In RegEx, you can also create a range of characters using dash. This ***pattern*** below is also equivalent:
...
The square brackets means the character must match one of the characters in set. We can add multiple square brackets to a search. In the following pattern we are looking for:
...
We can use ( - ) to create a range of all letters or numbers. This ***pattern*** will look for any file name with:

- an underscore ( `_` )

- followed by 2 letters ( `[a-zA-Z][a-zA-Z]` )

- followed by 1 number ( `[0-9]` )
...
An asterisk ( `*` ) says to repeat the preceding character between 0 and infinite time. However, we can find tune this to any range using curly brackets **`{ }`**

 

The curly bracket notation `{X,Y}` is the RegEx repeat operator that says to repeat the previous character or character set between X and Y times.
...
### Repeat ***exclusively*** X times

The above pattern does not care what comes before or after the 8 numbers – it could even be more numbers.

 

If we reduce it to 4 numbers in a row, then the pattern will match everything with 4 numbers in a row, including all the matches from above (because 8 numbers in a row matches the pattern of 4 numbers in a row.
...
However, if we want to match file names with [exactly four digits in a row]{.hl}, then we need to need to explicitly say that the four digits are preceded and followed by something that is not a number.

### The invert operator ( \^ )

The `^` has two roles in RegEx:

- At the beginning of a string it means [starts with]{.hl}

- At the beginning of a bracket it means [invert what is inside this bracket]{.hl}

 

The `^` operator is used in brackets to invert the pattern. `[^0-9]` means every character [except]{.hl} 0 to 9.

 

The ***pattern*** here is:

- a non-numeric character `[^0-9]`

- followed by 4 numeric characters `[0-9]{4}`

- followed by a non-numeric character `[^0-9]`
...
This code will look for any file name that has:

- a letter ( `[a-zA-Z]` )

- followed by 2 or 3 numbers ( `[0-9]{2,3}` )
...
- `{X,}`: minimum of X times (Y is assumed to be infinity)

- `{X,Y}`: between X and Y (inclusive of X and Y)

 

The most commonly used repeats have shortcuts:
...
If we specifically want to repeat AB 4 times, then we [group AB]{.hl}, by putting AB in parentheses [`(AB)`]{.hl}:
...
1\) In Comments answer: What will this pattern look for with the parentheses and without the parentheses?
...
3\) File names that has ***A#*** (i.e., A1, A2, A3...) and is from the year 2020. Assume that A# comes before the year.

 

4\) File names that have 25-28 characters and ends in ***.dat***
...
(the 2 should be mutually exclusive and cover all 75 filenames)
