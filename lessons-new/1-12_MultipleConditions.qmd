...
This lesson's data has two new columns in it compared to the previous lesson's data -- so 14 rows x ***7*** columns (instead of 5).  The new columns (***noonCondMessy*** and ***precipBad***) are copies of the previous columns with some issues added that are addressed in this lesson.
...
In this lesson we are going to learn how to create conditional statements with multiple conditions using two new operators: ***AND***, ***OR***.  The symbol for the ***AND*** operator is the ampersand ( ***&*** ) and the symbol for the ***OR*** operator is the vertical pipe ( **\|** ).
...
However, in English, it makes sense to ask: Is the day sunny or rainy?
...
``` r
  if(noonCond[i] == "Sunny" | "Rain")  # this is incorrect
```
...
[Note: This is a brute force method for finding multiple spellings.]{.note} There are more robust way to check for different spelling using substrings and pattern recognition (i.e., Regular Expressions), topics we will touch on later in this class.
...
So far when we have conditional operators to bound numbers on only one side – the other side goes to infinity.
...
Sometimes we want to check for values outside an expected range, usually this is an error check.  For instance, ***precipBad*** has some values that seem problematic:
...
The result for all three of these commands is a ***Boolean*** (a.k.a., ***logical*** or ***lgl***) vector with 14 values:

``` {.r tab="Variables"}
sunnyDayBool  FALSE FALSE TRUE FALSE...       lgl [14]
niceDayBool   FALSE FALSE FALSE FALSE...      lgl [14]
precipBool    FALSE FALSE FALSE TRUE...       lgl [14]
```
...
- ***sunnyDayBool*** subsetting ***X*** (the rows) – so only rows where ***sunnyDayBool*** is **TRUE** will be in the subsetted dataframe

- nothing subsetting ***Y*** (the columns) – so all columns will be in the subsetted dataframe
...
Make sure that every integer is accounted for [exactly once]{.hl} in the ***if-else-if*** structure. How you handle edge cases is up to you.
