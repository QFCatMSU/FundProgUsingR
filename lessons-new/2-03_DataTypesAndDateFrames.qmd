...
After you double-click on ***badData*** in ***Variables***, you can see there is an extra column in the data frame at the beginning which has the row numbers in it.  This is because, by default, ***write.csv()*** [will treat the row numbers as a column of data]{.hl}.
...
Since we are messing around with a throwaway data frame, let's see what happens if you try to add/overwrite a column to the data frame that has the wrong number of values.  In this case, the data frame has 366 rows, meaning there are 366 values in each column.

 

Let's attempt to:

- overwrite the column ***test1*** with **400** values and

- write to a new column called ***test2*** with **10** values:
...
Then R will create a column called ***test4*** that repeat the numbers **1-6** 61 times:
...
If we look at ***Variables*** (@fig-stringCol), there are a few columns in the ***weatherData*** data frame that R categorizes as string (***chr***) even though they are numeric by nature.  For instance, ***precip, snow,*** and ***snowDepth*** are all labelled as ***chr*** even though they have (mostly) numeric values.

 

The issue is the ***"T"*** values used in these columns to indicate a ***trace*** of precipitation (less than 0.01 inches).  ***"T"*** is a ***chr/string*** values and, in R, [if any value in a vector or a column is a string then the whole vector (column) is treated as a string]{.hl}.
...
One solution is to change the ***chr*** (string) vector into a numeric vector using ***as.numeric()***:
...
If we look at ***Variables***, we can see that ***precip2*** is a numeric vector and the numbers no longer have quotes around them.  Also, the non-numeric "T" values were replaced with ***NA***:

``` {.r tab="Variables"}
precip:   "0.04" "0.00" «"T" "T"» "0.00"   str [366]
precip2:  0.04 0 «NA NA» 0                 dbl [366]                
```
...
Whereas the ***NULL*** values are treated as if they do not exist:
...
``` {.r tab="Variables"}
precip3: 0.04 0.00 «0.005» «0.005» 0.00...   dbl [366]
```
...
The first plot will be a scatterplot with rainfall (***precipNum***) on the y-axis and day number on the x-axis.  Since day number is the row number, we will map the y-axis to a sequence from ***1*** to the number of rows in ***weatherData*** (i.e., **366**)
...
- In the ***weatherType*** column, there are a bunch of days without any conditions.  Would either NULL or NA be appropriate to use in those cells?  Explain. 
...
- March 21 June 21, and September 21 are approximate dates for the transition of the season -- it is OK for this application to be off by a couple days

- But, you need to deal with the fact that winter days occur at the beginning and end of the year
