...
- Clear all bp – including when you reload
- Sometimes you need to restart R Console to get bp to work
- breakpoints cannot be set while in debug mode (already an extension)
- Source while in browse mode increases \#
...
1.  A hollow arrow appears at the code line the script is paused at. The arrow shows you the line that will be executed next
...
In this lesson I will demonstrate using breakpoints within the lesson script.
...
- [BUG]{.hl}: broken – use Restart in ***Console*** (@fig-five_bp_controls)

6\) ***Disconnect***: quits the script (no more code is executed)

 

When in debug mode [you should only use the buttons to control the flow of your script]{.hl}.  Clicking ***Source*** while in debug mode will cause problems with your script. [Extension: Clicking Source in debug mode ].
...
- Click ***Step Out*** – this [should]{.hl} complete the ***for*** loop and pause execution at the next command after the ***for*** loop (line 29)
  ...
  - Current fix for bug: add breakpoint to line 29
...
The ***Restart*** button in the debug controls will also restart the R session and clean out ***Variables***, but breakpoints will not work after this (this is a bug as of Positron 2026.08). To fix breakpoints, click ***Restart*** in the ***Console*** tab.
...
If you keep clicking ***Step Over*** then debug mode will go through each command in the ***for*** loop for each cycle of the ***for*** loop (that could take a lot of time!). You can also click ***Continue***, but that will just take you to the breakpoint on the next cycle of the for loop.

### Step Out of the for loop

If you are inside a ***for*** loop and want to finish executing the whole ***for*** loop (i.e., complete every cycle of the loop), you can click ***Step Out***.  This will execute the rest of the ***for*** loop and put the control on the next command (in this case, the ***if-else*** structure). [Note: No bug here – the ***for*** loop and ***Step Out*** bug only occurs when there is a breakpoint in the ***for*** loop.]{.note}
...
***Step-in*** works whether the function is inside the same script or in another script.  However, ***Step-in*** will not work if the function is inside a package – we will deal with this next lesson.
...
D\) What would you need to do if, after the first time the conditional breakpoint in part C is hit, you want to complete the whole ***for*** loop and pause the script (i.e., pause right after the for loop completes).
