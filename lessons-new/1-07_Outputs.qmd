...
The backslash in programming is used in a string as an [escape character]{.hl}*.* An escape character tells R that [the next character is to be treated as a command]{.hl} rather than the character. This is similar to the way the **Shift**, **Control,** or **Alt** keys on your keyboard change the meaning of the character that is pressed with them (e.g, ***control-C*** is a command to copy).
...
- print a character that has a functional meaning like a double quote ( ***\\"*** ). In other words, treat the double quote as a character for output instead of the start or end of a string. This also works if you want to output a backslash ( **\\\\** )

- print our characters not on the keyboard using Unicode.  Here is a [link to a partial list of Unicode characters](https://www.rapidtables.com/code/text/unicode-characters.html).  Use the **Escape sequence** given on the page.  [Note: [according to Wikipedia](https://en.wikipedia.org/wiki/List_of_Unicode_characters), as of Sept 2025, there are almost 300,000 Unicode characters -- including an insane number of emojis (a recent addition).]{.note}

### Using backslash in Console

Here is an example of outputting special character using the backslash. This output includes tab (\\t), double quotes (\\"), newlines (\\n), backslash (\\\\), and a few Unicode characters.
...
Output the 14 values in the lowTemps variable
:::
...
Or, we can use the sequencing operator ( **:** ) and the sequencing function ***seq()*** to output multiple values in a vector:
...
And also add messages to the above output:
...
Type ?cat in the Console to open the Help for cat():

``` r
> ?cat
```

![Information about the cat() function in the Help tab.](images/R-1-06_CatHelp.png){#fig-catHelp .fs}

The first part of ***cat()*** is the three dots "**...**".  When 3 dots come before the arguments of a function, this means the function [will accept any number of objects]{.hl} as inputs.  The objects can be text in quotes, string variables, numeric variables, or other objects we have not discussed yet like ***Dates***.

 

When the three dots are elsewhere, as in ***read.csv()***, it means the function can be extended with arguments that are not on the list (this is an advanced topic).
...
The different components in ***cat()*** are now separated by " \*\* ".
...
By default, ***cat()*** will output to the ***Console***. But, you can have ***cat()*** output to a file using the ***file*** argument.  The ***file*** argument in ***cat()*** works just like the ***file*** argument in ***read.csv()*** with one exception:  if the file does not exist then R will create the file.
...
B. In the ***Help*** for the function ***c()***, the ***Default S3 method*** has 3 dots ( **...** ) as the first argument. In comments answer: What does the three dots tell you about the ***c()*** function?
...
- Challenge: output the total precipitation for the first 10 days (without using ***sum()***)

Make sure you use newlines (***\\n***) to make the output readable.

 

D. Using ***cat()***, output the data from part C to a file named ***output.txt*** in your ***data*** folder (instead of the ***Console***)

- Use the ***Help*** for ***cat()*** to figure out how to output to a file.

 

E. Challenge: Using ***cat()***, append your name and three different Unicode characters to the text in the ***output.txt*** file created in part D. 

- Again, use the ***Help*** for ***cat()*** to figure out how to add text to a file.\
   
...
The two main functions used to output to the ***Console*** are ***cat()*** and ***print()***. [The big difference is that ***print()*** does a lot more formatting of the output than ***cat()***.]{.hl}  You would likely use ***print()*** to display the results of a statistical test, which often has a complicated format.  ***cat()*** does very little formatting on its own, allowing the programmer more control over the output.  You would use this when you want to output text and some simple data or more control over formatting.
...
``` {.r tab="Console"}
> source("c:/Users/Charlie/Desktop/Test.R", echo = TRUE)
Error in `original_source()`:
! c:/Users/Charlie/Desktop/Test.R:«7:26: unexpected '\\'»
6: 
...
However, Positron has a built-in feature to help you find misplaced quotes -- the color scheme.  Quoted text is a different color than other parts of the code.  For example, in the default theme in Positron, quoted text is red.

 

![Missing quote in cat()](images/R-1-06_MissingQuote.png){#fig-missing-quote .fs}
...
Now the correct components of the output are in quotes and the script Sources without an error ([note: this is without echo]{.note}):

``` {.r tab="Console"}
> source("c:/Users/Charlie/Desktop/Test.R")
The values are... 
Distance:  100 
time:  50 
velocity:  2
...
This means R will [send the output from all script execution]{.hl} (no matter the script file) to the ***output2.txt*** file until you undo the command like this:
