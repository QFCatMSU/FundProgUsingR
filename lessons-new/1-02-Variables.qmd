...
In all programming languages there are 4 main structures that cover almost every aspect of programming:
...
1.  Open a Project in Positron (use the Folder created in the last lesson)
...
### Open Positron Project Folder

Use the project folder you created in the last lessons – the one you put on GitHub.

 

To open an Project Folder in Positron ([note: both options below will show recently opened Project Folders]{.note}) :

- Click ***Open***-\> ***Open Folder...*** -\> navigate to your Project Folder and click ***Select Folder***

- Use the Projects dropdown menu in the top-right corner of Positron

### Open a new script file

Inside your Project, click on ***New -\> New File -\> R File*** (@fig-openNewFile) .  An empty file named ***Untitled-1*** will appear as a tab in the File Editor window (never leave a file named ***Untitled***).  Save the file as ***lesson1-02.r*** in your ***script*** folder.

![Opening a new script file in Positron](images/R-1-02_OpeningScriptFile.png){#fig-openNewFile .fs}
...
Line **1** cleans out the **Variables** within the **Session** tab each time the script is executed (this is often called the ***Environment***). This provides you with a clean slate each time you run a script and I put this line at the top of most of my scripts.
...
Lines **1, 4, 5,** and **7** all have semicolons ( ***;*** ) at the end.  The semicolon designates the end of a programming command just like the period designates the end of a sentence.  The semicolon is optional in R and is often not used, but [I highly recommend the use of semicolons]{.hl} in your scripts. Semicolons allow for more flexibility (e.g., you can put multiple commands on one line), helps you think about the flow of your code, and are required in many programming languages.
...
### Execute your code -- the Session tab

Click ***Source*** to execute your R code ( @fig-vdtScript ).  Your Positron window should look similar to this:

![v, d, and t in the Session (Environment) after the script was Sourced](images/R-1-02_vdt_sourced-3.png){#fig-vdtScript .fs}

Notice that the **Session** tab displays the values for the variables ***v***, ***d***, and ***t***. The ***Session*** tab displays every variable in your script and is a very useful tool for viewing the results of your script and for debugging when things go wrong..

 

Try changing the values for ***d*** and ***t*** (lines 7 and 8) and click ***Source*** again to see how the values in the **Session** tab change.

#### dbl after variable values

In @fig-vdtScript ***dbl*** is put after the values in the Session tab. ***dbl*** stands for double ans is a type of numerical variable that we will talk about later.

### How to recover panes

Now is a good time to mention that it is pretty easy to accidentally close panes inPositron you did not mean to close. If you lose a pane and need it back you can go to ***View -\> Appearances*** to recover it:

![Recovering panes in Positron](images/PositronPanes.png){#fig-panes .fs}
...
In the **Session** tab the value of ***v*** remains ***5*** even though ***d*** changes. 

![Changing d in the script does not retroactively change v](images/R-1-02_change_d-2.png){#fig-change_d .fs}
...
![Assign a new value to v after assigning the new value to d](images/R-1-02_reassign_v-2.png){#fig-reassign_v .fs}
...
After your ***Source*** the script, make sure you can see all of the variables, with the correct answer, in the ***Session*** tab.

{{< include ../assets/_AppInstructions_Pos.qmd >}}

### Challenge

This challenge is only for those who have some programming experience already.
...
Put the above lines of code in your script individually and note the, often unintutive, error message that gets displayed in the **Console** tab.
...
![Assignment error in the R Script](images/R-1-02_error-assign-2.png){#fig-assignmentError .fs}

Notice that the execution of the script stops as soon as an error occurs -- so, in this case, line **9**, which calculates ***v***, never gets executed.
...
And you will see that you can reassign the value of ***pi*** in R. Most programming languages will give an error if you try to do this.

## Trap: Assigning nonexistent variables

The first time you assign a value to a variable, a storage container is created in memory and a value is put in it.  We call this a declaration (more about declarations later). After a variable has been declared, it can be used in calculations and reassigned values.  Before a variable is declared, R knows nothing about the variable.
...
You will see a lot of R programs written using the arrow.  In this class, I will use the equal sign because that is the standard for most other programming languages (including JavaScript, Python, and C) and it is easier to type.  You can use either operator in your scripts in this class.

 

For assigning values to a variable, ( ***\<-*** ) and ( **=** ) are functionally equivalent. There is a difference between the two operators when working with functions, which we will cover in a later lesson.

## Extension: Why clear the Environment tab?

There are many people who come to this class having already done some R programming and it is most likely they never clear the ***Environment*** (i.e., the ***Session*** -\> ***Variables*** tab) when running scripts. This means that variables are persistent and will still be around when another script is executed.
