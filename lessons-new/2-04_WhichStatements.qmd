...
The four index vectors [contain the index of the values that meet the condition]{.hl} -- not the values itself.

The number of values in the vector that met the condition is given by the number in brackets `int [X]`. The number is also the length of the vector.

``` {.r tab="Variables"}
index1: 2 4 5 7     int [4]
index2: 1 3 6       int [3]
index3: 1 4         int [2] 
index4: 2 3 5       int [3]
```
...
2.  subset other columns (e.g., ***avgTemp***, ***relHum***) by the index of these precipitation values

    - in other words, find the ***avgTemp*** or ***relHum*** on days where precipitation meets a certain condition
...
In ***Variables***, we see there are 60 **NA** values in ***precip2***, starting with values ***3, 4, 14***... (meaning the 3^rd^, 4^th^, 14^th^... values in ***precip2*** are **NA**)

``` {.r tab="Variables"}
naValues:  3 4 14 16 17 19 20 ...   num [60]
```

Create a copy of ***precip2*** called ***precip3*** (the copies are so we can see the progress in ***Variables***):
...
precip3:  0.04 0 «0.005» «0.005» 0 ...           dbl [366]
```
...
We see in ***Variables*** that there were:
...
::: {#fig-vectorIndices}
``` {.r tab="Variables"}
daysHighPrecip: 73 91 225 228 229 273 290 307           int [«8»]  
daysModPrecip:  33 88 119 121 123 125 168 183 ...       int [«18»]  
daysNoPrecip:   2 5 6 7 24 30 37 38 42 49 ...           int [«167»]  
```

Creating vector indices of days based on precipitation amounts
:::

### A note about decimals and best programming practice

When comparing values to 0 and 1, I coded it with decimal number:
...
The code would have worked just fine using integer numbers:

``` r
daysNoPrecip = which(weatherData$precipNum == «0»);   # days where precip is 0
daysHighPrecip = which(weatherData$precipNum > «1»);  # days where precip is > 1
```

It is good practice to put in the decimal (**1.00** as opposed to **1**) to indicate to the reader that the numbers being compared (in the precipitation column) are real numbers.

## Subsetting other vectors

The index vectors in @fig-vectorIndices gives the rows in ***weatherData\$precipNum*** that meet the given condition.  We can use this information to subset other columns and answer questions like:
...
***rainyDates*** and ***rainyDayWindSpeed*** are both vectors with 8 values, the 8 values are the ones on the rows that have precipitation greater than 1 inch.
...
In a previous lesson, we created a scatterplot by initializing a data frame in ***ggplot()*** and mapping columns to the ***x*** and ***y*** axes:
