...
Once in Date vector form, we can use which() to subset the vector by some Date value. In the first example we will subset the values in stnDate by the by month April. There are three different ways to subset for April and they will all produce the same results:
