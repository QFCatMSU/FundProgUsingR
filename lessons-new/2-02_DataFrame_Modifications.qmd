...
In Variables, you can see that ***weatherData*** consists of 366 observations (366 days -- it was a leap year) of 23 variables.  In other words there are 23 different weather variables in the data (columns) for each of the 366 days (rows).

``` {.r tab="Variables"}
weatherData:      [366 rows x 23 columns] <data.frame>
```

If we double-click on ***weatherData*** in ***Variables***, we can look at the data frame in the file viewer section of Positron:

![Viewing the weatherData data frame in the main section](images/R-1-14_ViewDataFrame.png){#fig-weatherData .fs}

[Note: the left section of the file viewer shows statistics for the columns – we will be ignoring that for the rest of this class.]{.note}
...
![The substr() function in Help ( `?substr`)](images/R-1-14_Substr.png){#fig-substr .fs}
...
![The paste() function -- with ... as the first "argument" ( `?paste` )](images/R-1-14_Paste-01.png){#fig-paste .fs}
...
The three dots represents a [pseudo-argument]{.hl}. This means that, for this function, [any argument that does not have a name]{.hl} will be pasted / concatenated (i.e., assigned to the three dots).
...
First, we will make a copy of ***weatherData*** called ***weatherData2***.  We will be creating multiple copies of ***weatherData*** so that we can see the progress of the data frames in ***Variables***.  You could just manipulate the original ***weatherData*** data frame.
...
Double-click on ***weatherData2*** in ***Variables*** and scroll to the end to see the new ***dateYear*** column:
...
The easiest way to delete a column from a data frame is to set the column to **NULL**.
...
There is no great way to move data frame columns around in R – you cannot just say "move column X to position Y".  Instead, [you need to recreate the column order of the data frame]{.hl} to reflect the new position of every column -- taking care to include every column.
...
( : ) is the [sequence operato]{.hl}r and it says to take all the columns in between (and inclusive of) ***maxTemp*** and ***windSusDir***.  Since ***maxTemp*** was the first column and ***windSusDir*** was the second to last column (the last being ***dateYear***), this basically says all other columns except ***dateYear***.

 

So, `select=c(dateYear, maxTemp:windSusDir))` says to order the columns with ***dateYear*** first and every other column after that.

![the dateYear column moves from the end to the beginning](images/R_2-02_MoveColumn.jpg){#fig-moveColumn .fs}
...
Let's says we want to move ***heatDays*** and ***coolDays*** right after ***tempDept.*** These are all columns in the middle of the data frame so you need to break the columns up to order them the way you want.
...
We are going to use the ***weatherData5*** data frame in the next lesson, so let's save it to a CSV file.
...
The above code, however, will add an extra column populated by the row numbers.  This is because, by default, ***write.csv()*** assumes the row numbers are row names.

 

To stop ***write.csv()*** from assuming the row numbers are row names, we need to set the argument ***row.names*** to **FALSE,** meaning that there are no row names in the data frame::
