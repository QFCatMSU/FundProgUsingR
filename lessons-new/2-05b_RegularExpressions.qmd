...
There are now 19 values, meaning that 7 file names had the substring ***reading*** or ***status***.
...
Finding substrings within a string is the most basic thing RegEx can do. We are going to step up in difficulty and check for something at the beginning and the end of a string. In RegEx, the caret ( \^ ) is a special character indicates the beginning of a string and ( \$ ) indicates the end of a string.
...
In this case, we need to be explicit about what is in between ***station*** and ***txt*** – which can by any number of any character. In Regex we use[ .\*]{.hl} to repesent any number of any characters.
...
### And has something in the middle
...
-   has any number of any any character ( ***.\**** )
...
We could put the escaped caret in a more complex pattern. In this case with use the caret as both a special character and an escape character:

``` r
has_caret2 = grep(file_names, pattern="^backup.*\\^ba")
```

In this case we want file names that starts with ***backup*** and have ***\^ba*** somewhere afterwards. There is just one file name with this pattern:
...
In Regex, a [dot ]{.hl}( . ) is a special character that [represents any character]{.hl}. We can also limit the character options but still allow for multiple characters.

 

We use square brackets \[ \] to indicate that we want to match exactly one character from the set specified inside the brackets.
...
The square brackets basically means the character must match one the characters in set. We can add multiple square brackets to a search. This indicate we are looking for:
...
We can use ( - ) to create a range of all letters. This will look for any file name with:
...
An asterisk ( \* ) says repeat the preceding character between 0 and infinite time. However, we can find tune to any range with curly brackets { }

 

The curly brackets {X,Y} are the RegEx repeat operator that says to repeat the previous character between X and Y times. If no value is put for Y, then the max is infinity.
...
However, if we want to only match file names with exactly four digits in a row, then we need to need to explicitly say that the the four digits and preceded and followed by something that is not a number.
...
Curly brackets tell regex to match the previous instruction:
...
It is hard to create one dataset that can cover all Regex scenario, so for this section I will just create a vector to demonstrate grouping. In this vector, A and AB are both repeated 1 through 5 times:
...
1\) What will this look for if you take out the parentheses?

``` r
repeat_AAB = grep(repeat_vals, pattern="(A[AB]){3}");
```

2\) Search for any file name with months between Jan (01) and May (5) and year between 2010 and 2023.
...
3\) A filename that has "A#" (i.e., A1, A2, A3...) and is from the year 2020

 

4\) Filename that has 15-20 characters and ends in .dat
