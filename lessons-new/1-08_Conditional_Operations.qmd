...
For the next couple of lessons we are just going to focus on ***if-else*** statements.  After that, we will introduce ***if-else*** statements embedded within ***for*** loops
...
Inside the curly brackets ( ***{  }*** ) attached to the ***if()*** is a [codeblock]{.hl}, or a series of commands, that gets executed if the conditional statement is **TRUE**.  If the statement is **FALSE**, the codeblock is ignored.
...
- @fig-ifExample where the curly brackets are on their own line and lined up at the same level as the ***if()*** and [all code inside is indented]{.hl}
...
[Note: This is not an efficient way to check multiple values. The more efficient way is to put the if-else inside a ***for()*** loop (in a later lesson).]{.note}
...
We are going to use conditional operators on the values in the ***noonCondition*** vector.  The ***noonCondition*** vector consists of character values, or, as they are more commonly called, [string values]{.hl} (labelled ***str*** in ***Variables***).  Since the value is a ***string***, we need to compare it to a value in quotes
...
This change the number 50 to the two characters: "5" and "0".  The ***if()*** statement above is also ***TRUE***, but for the wrong reason.  [Extension: Numbers as Characters]
...
But, this only works if [there is one command]{.hl} attached to each ***if()*** statement.  The curly brackets are always needed if there if more than one command. This author would not recommend removing brackets if you are a beginner, it can lead to problems down the road.
