...
A reminder that anytime a script file is looking for some resource, (e.g., a data file, another script file), the script needs a starting point (i.e., a folder) to find the location of the resource. R calls this starting point the Working Directory. [When you are in an Positron workspace, the starting folder/Working Directory is the Project Folder.]{.hl} This is why a project folder is easy to share – the file path used to link resources does not need to change when the folder is moved.
...
And we put the functions in the ***Variables*** tab by calling:

``` r
source("scripts/1-14_myFunctions.R");
```

![Functions loaded into the Session after sourcing the script for the lesson](images/R-1-14_FunctionsInVariables.png){#fig-functionsInVars .fs}
...
The modulus operator is often used in a ***for*** loop to perform a task at a regular interval. For instance, modulus can be used to check every 100^th^ value in a large vector:

`if(val %% 100 == 0)    # every 100th value has a mod of 0`
...
``` r
divBy0 = (remainder == 0); # TRUE if remainder is 0, FALSE otherwise
```
...
`(remainder == 0)` compares ***remainder*** to **0**, just like it would if inside ***if()*** statement. The results of this comparison is a Boolean ***TRUE/FALSE*** that is saved to the variable ***divBy0***.
...
[Note: ***return()*** ends a function so only one***return()***can be executed per function call.]{.note}
...
2\) Check to see if the argument is something other than numeric:
...
We are going to create a function that returns an undetermined number of values. Specifically, we are going to modify the ***isPrime1()*** to return all factors of the dividend supplied by the caller. For example, **12** can be divided by the factors **2, 3, 4**, and **6** so the return has 4 values: `c(2,3,4,6)`.
...
b.  How would you change ***findFactors()*** to return the number of factors instead the actual factors?
...
- Make ***isPrime1()*** more efficient by having the ***for*** loop cycle from 2 to the square root of ***dividend***
  - [note: the ***for*** loop will ignore the decimal in the square root value]{.note}
...
- The function returns all the numbers in the vector that can be evenly divided by at least one of 7, 11, or 13
...
- Have a default value for ***remainder***

- Have the function return an error if:

  - any of the three arguments are zero or negative

  - ***remainder*** is bigger than ***divisor***

 

6\) Create one function that converts a temperature value between the three temperature measurements: Celsius (C), Fahrenheit (F), and Kelvin (K).
...
You can combine the four error check into one conditional statement using OR ( **\|** ). The coding is simpler but the return message will not be as robust.
