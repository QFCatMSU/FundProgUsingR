...
As mentioned in the second lesson on variables, there are 4 structures that cover almost every aspect of programming:
...
In the next three lessons we cover ***for*** loops.  ***for*** loops allow you to write code that can cycle through multiple data points. 
...
There are two methods of cycling through a ***for*** loop: by ***value*** and by ***index***. By value is often taught first, but this class focuses on by index (this includes all examples in this lesson). For a detailed explanation go to: [Extension: Two methods of cycling]

## The index variable in a for loop

The true power of the ***for*** loop lies in the [indexing variable]{.hl}, [***i***]{.hl}.  [Note: the indexing variable can be given any name -- it's just common practice to name it ***i***.]{.note}  The indexing variable cycles through the sequence each time the ***for*** loop executes. This means the indexing variable can be used to modify the code every time the codeblock is executed.
...
So, if the sequence is **12:8** (a backwards sequence with 5 values), then the loop will cycle 5 time with:

- ***i*** set to **12** the first time the codeblock cycles

- ***i*** set to **11** the second time the codeblock cycles

- ***i*** set to **10** the third time the codeblock cycles

- ***i*** set to **9** the fourth time the codeblock cycles

- ***i*** set to **8** the fifth time the codeblock cycles
...
You do not have to name the index ***i***, you can name it whatever you want.  For instance, we will be working with weather so ***day*** might be a more appropriate/intutive name for the indexing variable.
...
Most examples so far use the index variable as a counter. The real power comes when you use the indexing variable to index values in a vector as we did in @fig-indexStringValues. This allowed us to perform an ***if-else*** statement on every value in a vector.
...
The code in @fig-IfElse-in-For is displaying specific information about each value in a vector.  This would get out of hand if there are thousands of values!  We usually want more general information about a vector -- for instance, we might want to know [how many days were sunny?]{.hl}
...
And we can look in ***Variables*** to see what the final value of ***sunnyDays*** was:
...
4\) Using a ***for*** loop, find out how many of the last 8 days were cloudy.

 

5\) Using a ***for*** loop, find the temperature difference for the 14 days
